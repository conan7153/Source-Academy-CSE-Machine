...
function scan_out_declarations(component) {
    if(is_sequence(component)){
        let decl_list = sequence_statements(component);
        while(!is_null(decl_list)){
            scan_out_declarations(head(decl_list));
            decl_list = tail(decl_list);
        }
    }else if(is_function_declaration(component)){
        fun_list = pair(declaration_symbol(component),
                        fun_list);
    }else if(is_declaration(component)){
        const_list = pair(declaration_symbol(component),
                        const_list);
    }else{}
    return append(fun_list, const_list);
}
